...
describe('Matrix Datas : [iris2d file] check matrix values', () => {
	...
	it('matrixValues should return valid datas, 2D PROB_TARGET_WITH_CELL index 2', () => {
		...
		[matrixFreqsValues, matrixValues, matrixExtras] = MatrixCanvasService.computeMatrixValues(
			...
			undefined,
			2);
		expect(matrixValues).toEqual([0.05405405405405406, 0.9523809523809523, 0, 0.75]);
	});
	...
	it('matrixValues should return valid datas, 2D PROB_CELL_WITH_TARGET index 2', () => {
		...
		[matrixFreqsValues, matrixValues, matrixExtras] = MatrixCanvasService.computeMatrixValues(
			...
			undefined,
			2);
		expect(matrixValues).toEqual([0.06451612903225806, 0.6451612903225806, 0, 0.2903225806451613]);
	});
	...
	it('matrixValues should return valid datas Regression PROB_CELL', () => {
		...
		[matrixFreqsValues, matrixValues, matrixExtras] = MatrixCanvasService.computeMatrixValues(
			...
			undefined,
			-1);
		expect(matrixValues).toEqual([1, 0, 0, 0, 0.8648648648648649, 0.13513513513513514, 0, 0, 1]);
	});
	...
	it('matrixValues should return valid datas Regression PROB_CELL_REVERSE', () => {
		...
		[matrixFreqsValues, matrixValues, matrixExtras] = MatrixCanvasService.computeMatrixValues(
			...
			undefined,
			-1);
		expect(matrixValues).toEqual([1, 0, 0, 0, 1, 0.1388888888888889, 0, 0, 0.8611111111111112]);
	});
	...
	it('matrixValues should return valid datas Regression MUTUAL_INFO', () => {
		...
		[matrixFreqsValues, matrixValues, matrixExtras] = MatrixCanvasService.computeMatrixValues(
			...
			undefined,
			-1);
		expect(matrixValues).toEqual([0.36358699005184597, 0, 0, 0, 0.31812587878476467, -0.04999389336018405, 0, 0, 0.31676392183871593]);
	});
	...
	it('matrixExtras should return valid datas Regression MUTUAL_INFO', () => {
		...
		[matrixFreqsValues, matrixValues, matrixExtras] = MatrixCanvasService.computeMatrixValues(
			...
			undefined,
			-1);
		expect(matrixExtras).toEqual([false, true, true, true, false, false, true, true, false]);
	});
