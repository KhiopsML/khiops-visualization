...
import {
	CopyDatasService
} from '@khiops-library/providers/copy-datas.service';
...
describe('PreparationDatasService', () => {
	...
	it('copyDatasToClipboard should return valid datas [Natives-Arbres_Paires_AllReports, getInformationsDatas]', () => {
		const currentSelectedArea = {
			...
			inputDatas: [{
				...
				'value': 'C:\\Users\\yfco6286\\Desktop\\LAROSE Cyril\\essai_07\\20210303.result.models.021.csv'
			}, {
	...
	it('copyDatasToClipboard should return valid datas [adult-bivar, getNdBarChartDatas]', () => {
		...
		const intervalDatas = copyDatasService.getNdBarChartDatas(currentSelectedArea);
		const expectedRes = 'Target variable stats\nless\tmore\t\n0.7616316497922397\t0.2383683502077603\t\n';
		expect(JSON.stringify(intervalDatas)).toEqual(JSON.stringify(expectedRes));
