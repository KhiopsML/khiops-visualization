...
import {
	HistogramService
} from "@khiops-visualization/components/commons/histogram/histogram.service";
import {
	AppService
...
function getHistogramGraphDatas(variable) {
	const fileDatas = require('../mocks/visualization/ylogAdultAllReports.json');
...
describe("Visualization", () => {
	describe("Histogram datas", () => {
		...
		it('getRangeX should return valid datas [ylogAdultAllReports, R3]', () => {
			const histogramDatas = getHistogramGraphDatas(2)
			const res = histogramService.getRangeX(histogramDatas)
		...
		it('computeXbarDimensions should return valid datas [ylogAdultAllReports, R1]', () => {
			const histogramDatas = getHistogramGraphDatas(0)
			const res = histogramService.computeXbarDimensions(histogramDatas, 'xLin')
			expect(JSON.stringify(res[0])).toEqual(JSON.stringify({
				"barWlog": 0,
				"barXlog": 0,
				"barWlin": 1,
				"barXlin": 0,
		...
		it('computeXbarDimensions should return valid datas [ylogAdultAllReports, R2]', () => {
			const histogramDatas = getHistogramGraphDatas(1)
			const res = histogramService.computeXbarDimensions(histogramDatas, 'xLin')
			expect(JSON.stringify(res[0])).toEqual(JSON.stringify({
				"barWlog": 0,
				"barXlog": 0,
		...
		it('computeXbarDimensions should return valid datas [ylogAdultAllReports, R1]', () => {
			const histogramDatas = getHistogramGraphDatas(0)
			const res = histogramService.computeXbarDimensions(histogramDatas, 'xLog')
			expect(JSON.stringify(res[0])).toEqual(JSON.stringify({
