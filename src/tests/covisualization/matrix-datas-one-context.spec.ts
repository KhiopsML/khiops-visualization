...
describe('Matrix Datas : One Context [zero-except file]', () => {
	...
	it('Cell[0] cellHellinger should return valid datas', () => {
		expect(cells[0].cellHellingerValue).toEqual([0.08251563190214278, -0.2721655269759087]);
	});
	it('Cell[0] cellHellinger should return valid datas', () => {
		expect(cells[0].cellHellingerAbsoluteValue).toEqual([0.0068088295082099235, 0.07407407407407407]);
	});
	...
	it('Cell[1] cellHellinger should return valid datas', () => {
		expect(cells[1].cellHellingerValue).toEqual([0.048207140552315175, -0.31426968052735443]);
	});
	it('Cell[1] cellHellinger should return valid datas', () => {
		expect(cells[1].cellHellingerAbsoluteValue).toEqual([0.00232392840023067, 0.09876543209876541]);
	});
...
describe('Matrix Datas : one Context [zero-except file] check proba values without folding', () => {
	...
	it('matrixValues should return valid datas', () => {
		expect(matrixValues).toEqual([0.5714285714285714, 0.42857142857142855, 0, 0.16666666666666666, 0.3333333333333333, 0.5, 0.4, 0.2, 0.4]);
	});
});
...
describe('Matrix Datas : one Context [zero-except file] check proba values with folding', () => {
	...
	it('matrixValues should return valid datas', () => {
		expect(matrixValues).toEqual([0.36363636363636365, 0.2, 0.6363636363636364, 0.8]);
	});
});
...
describe('Matrix Datas : one Context [adultmissing file] check rev proba values with complex folding', () => {
	...
	it('matrixValues should return valid datas', () => {
		expect(matrixValues).toEqual([0.339662447257384, 0.011421041071820778, 0.14737793851717904, 0.18075993850208655, 0.3297166968053044, 0.10806061937184275, 0.015030530765617662, 0.15594175669328322, 0.09581963363081258, 0.15702230259192285, 0.520755545793982, 0.5246594645373415, 0.026220614828209764, 0.17900285526026796, 0.20854861437294506]);
	});
});
