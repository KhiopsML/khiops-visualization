...
describe('SaveService - truncateJsonHierarchy', () => {
  ...
  beforeEach(() => {
    const dimensionsDatasServiceSpy = jasmine.createSpyObj('DimensionsDatasService', [], {
      dimensionsDatas: {
        selectedDimensions: [],
        dimensionsClusters: [],
      },
    ...
    const layoutServiceSpy = jasmine.createSpyObj('LayoutService', ['getSplitSizes']);
    const viewManagerServiceSpy = jasmine.createSpyObj('ViewManagerService', ['getViewsLayout']);
    const treenodesServiceSpy = jasmine.createSpyObj('TreenodesService', ['getUnfoldHierarchy']);
    const annotationServiceSpy = jasmine.createSpyObj('AnnotationService', ['getAnnotations']);
    const importExtDatasServiceSpy = jasmine.createSpyObj('ImportExtDatasService', ['getImportedDatas']);
  ...
  it('Test 3: should create new hierarchy array when processing', () => {
    ...
    expect(result.coclusteringReport.dimensionHierarchies).not.toBe(originalHierarchy);
  });
  ...
  it('Test 4: should handle invalid dimension name gracefully', () => {
    ...
    const result = service.truncateJsonHierarchy(mockData);
    expect(result.coclusteringReport.dimensionHierarchies[0].clusters.length).toBe(3);
  });
  ...
  it('Test 7: should preserve cluster properties during sorting', () => {
    ...
    mockData.coclusteringReport.dimensionHierarchies[0].clusters[0].customProp = 'test';
    
    const result = service.truncateJsonHierarchy(mockData);
    const sortedCluster = result.coclusteringReport.dimensionHierarchies[0].clusters.find(c => c.customProp === 'test');
  ...
  it('Test 8: should handle large datasets efficiently', () => {
    ...
    mockData.coclusteringReport.dimensionHierarchies[0].clusters = largeClusters;
    mockData.savedDatas.collapsedNodes = { 'dimension1': [] };
    ...
    expect(result.coclusteringReport.dimensionHierarchies[0].clusters.length).toBe(1000);
  });

  it('Test 9: should maintain sorting invariants', () => {
    const mockData = createMockData();
    mockData.savedDatas.collapsedNodes = { 'dimension1': [] };
    
    // Mix up the order
    mockData.coclusteringReport.dimensionHierarchies[0].clusters = [
      { cluster: 'leaf_high', isLeaf: true, rank: 10 },
      { cluster: 'parent_low', isLeaf: false, rank: 1 },
      { cluster: 'leaf_low', isLeaf: true, rank: 1 },
      { cluster: 'parent_high', isLeaf: false, rank: 10 }
    ];

    const result = service.truncateJsonHierarchy(mockData);
    const clusters = result.coclusteringReport.dimensionHierarchies[0].clusters;
    
    // Verify sorting invariant: with lodash sortBy([e => e.isLeaf === false, 'rank'])
    // All leaves (isLeaf=true) come first, then all non-leaves (isLeaf=false)
    let foundNonLeaf = false;
    for (const cluster of clusters) {
      if (!cluster.isLeaf) {
        foundNonLeaf = true;
      } else if (foundNonLeaf) {
        fail('Found leaf after non-leaf - sorting invariant violated');
      }
    }
  });
