				...
				try {
					...
					html2canvas(currentDiv)
						.then((canvas) => {
							...
							currentDiv.parentNode.classList.contains(
								"printing",
							) &&
								currentDiv.parentNode.classList.remove(
									"printing",
								) &&
						...
						.catch((e) => {
							console.error(
								"​HeaderToolsComponent -> copyImage -> e",
								e,
							);
							this.snackBar.open(
