	...
	resizeGraph(firstRender = false) {
		if (this.inputDatas) {
			...
			if (graphWrapper && graphWrapper.children[0]) {
				...
				let compWidth =
					this.inputDatas.labels.length * this.scaleValue * 0.6;
				...
				if (elCanvas) {
					...
					elCanvas.style.height =
						parentHeight - bottomScrollbar + "px";
					elMonitor.style.height =
						parentHeight - bottomScrollbar + "px"; // In case of height resize
					if (!firstRender) {
