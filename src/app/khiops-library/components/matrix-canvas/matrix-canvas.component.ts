	...
	implements OnChanges
{
						...
						if (this.matrixDiv && this.matrixArea) {
							...
							if (this.inputDatas.matrixCellDatas) {
								...
								if (!this.minMaxValues) {
									...
									// For KV purpose
									[minVal, maxVal] =
										this.minMaxValues[this.graphMode.mode];
								}
								...
								this.xAxisLabel =
									this.inputDatas.variable.nameX;
								this.yAxisLabel =
									this.inputDatas.variable.nameY;
								...
								const cellsLength =
									this.inputDatas.matrixCellDatas.length;
								this.matrixCtx.beginPath();
								this.matrixCtx.strokeStyle =
								...
								for (
									...
									const cellDatas =
										this.inputDatas.matrixCellDatas[index];
									...
									cellDatas.displayedValue = {
										...
										value: currentVal,
										ef: this.matrixExpectedFreqsValues[
											index
										],
										extra:
											(this.matrixExtras &&
												this.matrixExtras[index]) ||
											0,
									};
									cellDatas.displayedFreqValue =
										this.matrixFreqsValues[index];
			...
			if (this.selectedCell) {
				...
				for (let index = 0; index < cellsLength; index++) {
					...
					if (
						...
						this.selectedNodes?.[1]?.childrenList.includes(
							cellDatas.yaxisPart,
						)
					) {
	...
	zoomCanvas(delta, preventTranslate = false) {
		...
		if (previousZoom !== this.zoom) {
			...
			if (!preventTranslate) {
				this.matrixArea.nativeElement.scrollLeft =
					this.currentMouseX * this.zoom -
					this.currentMouseX -
					deltaX;
				this.matrixArea.nativeElement.scrollTop =
					this.currentMouseY * this.zoom -
					this.currentMouseY -
					deltaY;
			}
	...
	getColorForPercentage(currentColorVal, maxVal) {
		...
		if (currentColorVal === 0) {
			...
			if (
				...
				percentColors =
					MatrixCanvasService.getInterestColors(isPositiveValue);
			...
			const rgba =
				"rgba(" + [color.r, color.g, color.b, 1].join(",") + ")";
