...
export class DimensionsDatasService {
	...
	initSavedDatas() {
		...
		const savedMatrixContrast =
			this.appService.getSavedDatas("matrixContrast");
		...
		const savedIsAxisInverted =
			this.appService.getSavedDatas("isAxisInverted");
	...
	toggleIsAxisInverted() {
		this.dimensionsDatas.isAxisInverted =
			!this.dimensionsDatas.isAxisInverted;
	}
		...
		if (!treeNode.isCollapsed) {
			if (treeNode.children.length > 0) {
				for (let i = 0; i < treeNode.children.length; i++) {
					count = this.getNodeIntervalsCount(
						treeNode.children[i],
						count,
					);
				}
			...
			for (let i = 0; i < selectedDimensionsLength; i++) {
				...
				if (dimensionHierarchy) {
					...
					for (let j = 0; j < clustersLength; j++) {
						...
						this.dimensionsDatas.dimensionsClusters[i].push(
							currentObj,
						);
				...
				if (currentDimensionHierarchy) {
					...
					// First convert each child into a treenode value object
					const clustersLength =
						currentDimensionHierarchy.clusters.length;
					for (let j = 0; j < clustersLength; j++) {
						...
						this.dimensionsDatas.currentDimensionsClusters[i].push(
							currentObj,
						);
	...
	getMatrixDatas(propagateChanges = true) {
		...
		for (
			let i = 2;
			i < this.dimensionsDatas.dimensionsClusters.length;
			i++
		) {
