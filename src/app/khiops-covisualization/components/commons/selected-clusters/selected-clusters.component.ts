...
export class SelectedClustersComponent implements OnDestroy {
	...
	updateDimensionIntervals(dimName) {
		if (this.selectedClusters) {
			...
			const nbClusters =
				this.dimensionsService.getDimensionIntervals(dimName);

			// Just update intervals on change
			const selectedCluster: SelectedClusterVO =
				this.selectedClusters.find((e) => e.hierarchy === dimName);
			if (selectedCluster) {
	...
	updateClustersInformations() {
		...
		if (
			...
			this.selectedNodes.length ===
				this.dimensionsService.getDimensionCount()
		) {
	...
	selectActiveClusters() {
		...
		const secondDimPos =
			this.dimensionsService.getDimensionPositionFromName(
