...
export class HomeLayoutComponent implements OnInit, OnDestroy {
			...
			this.eventsService.importedDatasChanged.subscribe((dimName) => {
				if (dimName && dimName[0]) {
					...
					this.treenodesService.setSelectedNode(
						this.dimensionsService.dimensionsDatas
							.selectedDimensions[dimIndex].name,
						this.treenodesService.dimensionsDatas.selectedNodes[
							dimIndex
						]._id,
						false,
					);
	...
	openLoadExternalDataDialog() {
		...
		config.maxWidth =
			AppConfig.covisualizationCommon.MANAGE_VIEWS.MAX_WIDTH;
