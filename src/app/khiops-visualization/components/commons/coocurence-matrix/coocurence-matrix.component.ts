...
import {
	MatrixTargetsI
} from '@khiops-library/interfaces/matrix-targets';
import {
	MatrixOptionsI
...
export class CoocurenceMatrixComponent implements OnChanges, AfterViewInit {
	...
	constructor(public selectableService: SelectableService,
		...
		this.minMaxValues = this.preparation2dDatasService.getGlobalMinAndMax2dValues(this.preparation2dDatasService.getVariablesd2Datas());
	...
	ngOnChanges(changes: SimpleChanges) {
		...
		if (changes.selectedVariable &&
			...
			this.matrixCells = this.preparation2dDatasService.getMatrixCoocurenceCellsDatas();
	...
	constructModeSelectBox() {
		...
		if (this.matrixTargets.targets.length > 0) {
			this.matrixModes.types = [{
				...
				title: 'I (' + this.translate.get('GLOBAL.TARGET') + ' | ' + varName2 + ' , ' + varName1 + ')'
			}, {
				mode: 'FREQUENCY',
				title: 'Frequency'
			}, {
				mode: 'FREQUENCY_CELL',
				title: 'Frequency (' + this.translate.get('GLOBAL.TARGET') + ' | ' + varName2 + ' , ' + varName1 + ')'
			}, {
				mode: 'PROB_TARGET_WITH_CELL',
				title: 'P (' + this.translate.get('GLOBAL.TARGET') + ' | ' + varName2 + ' , ' + varName1 + ')'
			}, {
				mode: 'PROB_CELL_WITH_TARGET',
				title: 'P (' + varName2 + ' , ' + varName1 + ' | ' + this.translate.get('GLOBAL.TARGET') + ')'
			}, {
				mode: 'CELL_INTEREST',
	...
	constructTargetSelectBox() {
		...
		if (this.preparation2dDatasService.getTargetsIfAvailable()) {
			this.matrixTargets.targets = this.preparation2dDatasService.getTargetsIfAvailable();
		} else {
	...
	selectTargetSelectBox(selected = null) {
		...
		if (this.preparation2dDatasService.getTargetsIfAvailable() &&
			...
			this.matrixTargets.selected = selected || previousSelectedTarget || this.matrixTargets.targets[0];
		} else {
			this.matrixTargets.selected = undefined;
		}
	}
