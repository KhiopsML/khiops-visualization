...
export class DistributionDatasService {
  ...
  initTargetDistributionDisplayedValues(partition: string[] | number[][]) {
    ...
    const partitionNames = partition?.map(formatLabel);

    if (JSON.stringify(currentNames) !== JSON.stringify(partitionNames)) {
  ...
  private getAutoScaleFactor(): number {
    ...
    if (stored !== null && stored !== undefined && stored !== '') {
      ...
      if (
        !isNaN(parsed) &&
        parsed >= AppConfig.visualizationCommon.GLOBAL.AUTO_SCALE_THRESHOLD_MIN &&
        parsed <= AppConfig.visualizationCommon.GLOBAL.AUTO_SCALE_THRESHOLD_MAX
      ) {
        return parsed;
      }
    }
    return AppConfig.visualizationCommon.GLOBAL.AUTO_SCALE_THRESHOLD_DEFAULT;
  }
