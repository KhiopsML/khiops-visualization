...
export class TreePreparationDatasService {
	...
	initialize() {
		...
		if (this.treePreparationDatas.isValid()) {
			...
			const savedSelectedRank =
				this.appService.getSavedDatas("selectedRank");
				...
				if (variableDetails && variableDetails.dataGrid) {
					...
					if (currentVariableType === TYPES.NUMERICAL) {
						...
						if (
							...
							datas[0]["interval"] = JSON.stringify(
								variableDetails.dataGrid.dimensions[0]
									.partition[index],
							);
						} else {
							datas[0]["interval"] =
								this.translate.get("GLOBAL.MISSING");
						}
						...
						const partLength =
							variableDetails.dataGrid.dimensions[0].partition
								.length;
						const isMultiDimPartition =
							partValuesLength !== partLength;
						...
						for (let i = startIter; i < dimensionLength; i++) {
							...
							datas[i - startIter]["frequency"] =
								variableDetails.inputValues.frequencies[
									currentValueIndex
								];
						}
					...
					if (variableDetails.isLimitedDatas) {
						title +=
							" ( * " +
							this.translate.get(
								"GLOBAL.LIMIT_GRAPH_DATAS_WARNING",
							) +
							")";
					}
				}
	...
	getTargetVariableStatsDatas(): ChartDatasVO {
		...
		if (
			...
			const currentDatas =
				appDatas.treePreparationReport.summary.targetValues;
			...
			for (const item in appDatas.treePreparationReport.summary
				...
				if (item) {
					informationsDatas.push({
						title: item,
						value: appDatas.treePreparationReport.summary
							.targetDescriptiveStats[item],
					});
				}
			}
		}
	...
	toggleNodeIntoDimenstionTree(node: TreeNodeVO, nodeToFind, parent?, i?) {
		if (node && node.nodeId === nodeToFind) {
			if (parent.children[i].children) {
				...
				parent.children[i].collapsedChildren =
		...
		if (node && node.children && node.children.length > 0) {
			for (let i = 0; i < node.children.length; i++) {
				if (node.children[i]) {
					const res = this.toggleNodeIntoDimenstionTree(
						node.children[i],
						nodeToFind,
						node,
						i,
				...
				if (currentNode && currentNode.isLeaf) {
					...
					const rowData: any = {
						...
						frequencies: JSON.stringify(
							currentNode.targetValues.frequencies,
						),
					};
	...
	getTreeLeafRules(): GridDatasI {
		...
		if (this.treePreparationDatas.selectedNodes[0]) {
			...
			if (nodeHierarchy) {
				rules = this.getRecursiveNodeDatasRules(
					nodeHierarchy[0],
					rules,
				);
			}
