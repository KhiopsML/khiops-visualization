...
export class SaveService {
	...
	constructDatasToSave() {
		...
		const selected2dRank = this.preparation2dDatasService.getSelectedVariableRank();
		const selected2dCell = this.preparation2dDatasService.getSelectedCellIndex();
		appDatas.savedDatas = new SavedDatasVO(splitSizes, selectedRank, selected2dRank, selected2dCell);
		return appDatas;
	}
