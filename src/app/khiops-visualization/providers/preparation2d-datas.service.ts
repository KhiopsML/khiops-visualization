...
export class Preparation2dDatasService {
	...
	setSelectedVariable(object: any): Preparation2dVariableVO {
		if (object) {
			const variable = this.getVariableFromNames(
				object.name1,
				object.name2,
			);
			if (variable) {
				this.preparation2dDatas.selectedVariable =
					new Preparation2dVariableVO(variable);
		...
		if (
			...
			if (currentDatas) {
				for (let i = 0; i < currentDatas.length; i++) {
					const varItem: Variable2dVO = new Variable2dVO(
						currentDatas[i],
					);
	...
	getCurrentCellDatas(): {
		...
		if (selectedVariable) {
			const variableDetails = this.getVariableDetails(
			...
			if (selectedCell && variableDetails && variableDetails.dataGrid) {
				...
				if (xType === TYPES.NUMERICAL) {
					displayedColumnsX.push({
						headerName:
							this.translate.get("GLOBAL.INTERVAL_OF") + xName,
						field: "interval",
					});
				} else {
					displayedColumnsX.push({
						headerName:
							this.translate.get("GLOBAL.VALUES_OF") + xName,
						field: "values",
					});
					displayedColumnsX.push({
				...
				if (yType === TYPES.NUMERICAL) {
					displayedColumnsY.push({
						headerName:
							this.translate.get("GLOBAL.INTERVAL_OF") + yName,
						field: "interval",
					});
				} else {
					displayedColumnsY.push({
						headerName:
							this.translate.get("GLOBAL.VALUES_OF") + yName,
						field: "values",
					});
					displayedColumnsY.push({
						headerName: this.translate.get("GLOBAL.FREQUENCY"),
	...
	getMatrixCanvasDatas(selectedVariable): any {
		...
		if (variablesDetails) {
			...
			const variableDatas: VariableDetailsVO =
				_.cloneDeep(variablesDetails);
			...
			if (variableDatas) {
				...
				if (
					...
					variableDatas.setTargetCellFrequencies(
			...
			for (let j = 0; j < yLength; j++) {
				for (let i = 0; i < xLength; i++) {
					if (datas.partTargetFrequencies) {
						// Regression case
						datas.cellFrequencies.push([
							datas.partTargetFrequencies[i][j],
						]);
					}
				}
			}
	...
	getGlobalMinAndMax2dValues(
		...
		res["MUTUAL_INFO_TARGET_WITH_CELL"] =
