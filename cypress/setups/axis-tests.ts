export function setupAxisTests(datas, testsValues, fileName) {
  ...
  testsValues.Axis.push(datas.coclusteringReport?.dimensionSummaries[1]?.name);
  ...
  if (
    datas.coclusteringReport?.dimensionSummaries[0].type === 'Categorical' ||
  ...
  testsValues.Axis.push('Coverage');
