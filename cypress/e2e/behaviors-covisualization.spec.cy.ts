...
describe('Behaviors tests for Khiops Covisualization', () => {
  ...
  files.forEach((fileName, fileIndex) => {
    it(`Test behaviors clicks`, () => {
      ...
      // Check values
      cy.get('.matrix-tooltip-comp').contains('Self');
      cy.get('#cluster-distribution-1').contains('Self');
      cy.get('#selected-clusters-grid').contains('Self');
      cy.get('#cluster-composition-0').contains('Self');
      cy.get('.annotation-comp').first().contains('Self');

      // To do : aggrid click is not propagated
      // ### Click on first cluster
      // cy.get('#cluster-details-grid-0 [row-id="0"]')
      //   .first()
      //   .click({ force: true });
