...
export function dfs2({ node, abortFilter, preAction, highway, idx = 0 }) {
	if (!node) return;
	if (!abortFilter(node, idx, highway)) return;
	if (preAction) preAction(node, idx, highway);
	if (node.children)
		for (var i = 0; i < node.children.length; i++) {
			var h = highway;
			if (highway.length > 1)
				if (node.children[i] == highway[1]) h = highway.slice(1);
			dfs2({
...
export function compose(t1: T, t2: T): T {
	...
	var θ = CdivC(
		CaddC(CmulC(t1.θ, t2.θ), CmulC(t1.θ, CmulC(Ccon(t1.P), t2.P))),
		divisor,
	);
	return {
...
var CkaddC = (a: Ck, b: Ck) => ({
	re: a.re + b.re,
	im: a.im + b.im,
});
var CksubCk = (a: Ck, b: Ck) => ({
	re: a.re - b.re,
	im: a.im - b.im,
});
var CkmulR = (p: Ck, s: number) => ({
	re: p.re * s,
	im: p.im * s,
});
var CkmulCk = (a: Ck, b: Ck) => ({
