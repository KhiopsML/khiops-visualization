...
class RDFHelper {
  static getIdFromScheme(scheme: Object): string {
    ...
    for (let i = 0; i < schemeProperties.length; i++) {
      if (schemeProperties[i].indexOf(':identifier') !== -1) {
        ...
        if (identifier.hasOwnProperty('0')) identifier = identifier[0];
        return identifier;
      }
    }
  ...
  static getNameFromScheme(scheme: Object): string {
    ...
    for (let i = 0; i < schemeProperties.length; i++) {
      if (schemeProperties[i].indexOf(':title') !== -1) {
        let identifier = scheme[schemeProperties[i]];
        if (identifier.hasOwnProperty('0')) identifier = identifier[0];
        if (identifier.hasOwnProperty('_')) identifier = identifier['_'];
        return identifier;
      }
    }
  ...
  static getIdFromConcept(concept: Object): string {
    let id = null;
    if (concept.hasOwnProperty('$')) concept = concept['$'];
    if (concept.hasOwnProperty('rdf:about')) id = concept['rdf:about'];
  ...
  static getNameFromConcept(concept: Object): string {
    ...
    if (concept.hasOwnProperty('skos:prefLabel')) {
      ...
      if (concept.hasOwnProperty('0')) concept = concept['0'];
      if (concept.hasOwnProperty('_')) concept = concept['_'];
    ...
    for (let i = 0; i < conceptProperties.length; i++) {
      ...
      if (
        currProperty.indexOf(':title') !== -1 ||
        currProperty.indexOf('Label') ||
        currProperty.indexOf('Name') !== -1
      ) {
        let name = concept[currProperty];
        if (name.hasOwnProperty('0')) name = name[0];
        if (name.hasOwnProperty('_')) name = name['_'];
        return name;
      }
    }
...
class InputTreeML {
  ...
  private static toJSON(inputNode: Object): Object {
    ...
    if (resultNode.id == '') resultNode.id = resultNode.name;

    //children
    if (inputNode.hasOwnProperty('branch')) {
      if (resultNode.children == null) resultNode.children = [];
...
class InputSkos {
  ...
  private static addChildIds(conceptList: Object[]): TreeNode[] {
    ...
    for (let i = 0; i < conceptList.length; i++) {
      ...
      if (resultArray.indexOf(node) == -1) resultArray.push(node);
    }
    return resultArray;
  }
  ...
  private static addOtherNodes(root: TreeNode, conceptList: Object[]) {
    ...
    for (let i = 0; i < conceptList.length; i++) {
      ...
      if (currentConcept.hasOwnProperty('skos:inScheme')) {
        node.parent = this.addParentId(
          currentConcept['skos:inScheme'][0]['$']['rdf:resource'],
        );
      ...
      for (let k = 0; k < nodesWithUndefinedParents.length; k++) {
        if (
          root.addChildToScheme(
            nodesWithUndefinedParents[k],
            nodesWithUndefinedParents[k].parent.id,
          ) == true
        )
  ...
  private static connectNodesFromMap(map: {
    ...
    for (let nodeId in map) {
      if (map[nodeId].children && !map[nodeId].parent) {
        if (
          nodeMaxChildren == null ||
          nodeMaxChildren.children.length < map[nodeId].children.length
        )
...
export class TreeNode {
  ...
  addChild(child: TreeNode) {
    ...
    else this.children[childIndex] = child;
  }
  ...
  setChild(node: TreeNode) {
    ...
    for (let i = 0; i < this.children.length; i++) {
      if (this.children[i].setChild(node) == true) return true;
    }
    return false;
  }
