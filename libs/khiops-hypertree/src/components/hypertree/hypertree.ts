...
export class Hypertree {
		...
		toggleSelection: (n: N) => {
			...
			if (this.args.objects.pathes.length > 10 + 1) {
				...
				this.args.objects.selections =
					this.args.objects.selections.filter((e) => e !== toremove);
	...
	protected updateUnitdiskView() {
		...
		this.unitdisk = new this.args.geometry.decorator(
			...
			},
			{
				...
				transform: (n: N) =>
					this.unitdisk.args.transformation.transformPoint(
						n.layout.z,
					),
	...
	protected initData(d3h, t0, t1, dl) {
		...
		var ncount = 1;
		globelhtid++;
		this.data = <N & d3.HierarchyNode<N>>(
			d3.hierarchy(d3h).each((n: any) => {
		...
		this.args.geometry.transformation.cache.N =
			this.data.descendants().length;
	...
	protected updateWeights_(): void {
		...
		this.sum(
			this.data,
			this.args.filter.weightFilter.weight,
			"cullingWeight",
		);
		...
		const updateLabelLen_ = (txtprop, lenprop) => {
			this.data.each((n) => {
				if (n.precalc[txtprop]) {
					...
					n.precalc[lenprop] =
						metrics.width / 200 / window.devicePixelRatio;
	...
	protected setPathHead(path: Path, n: N) {
		...
		const oldPath = this.args.objects.pathes.find(
			(e) => e.id === oldPathId,
		);
	...
	protected removePath(pathType: string, n: N) {
		try {
			...
			if (this.args.objects.pathes) {
				...
				n.ancestors().forEach((pn: N) => {
					if (pn.pathes.partof) {
						pn.pathes.partof = pn.pathes.partof.filter(
							(e) => e.id !== pathId,
						);
