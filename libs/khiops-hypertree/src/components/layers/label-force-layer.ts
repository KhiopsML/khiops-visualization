...
export class LabelForceLayer implements ILayer {
	...
	update = {
		...
		force: () => {
			...
			if (
				this.view.hypertree.isAnimationRunning() &&
				this.args.hideOnDrag
			)
				return;

			this.labelSetUpdate();
			for (let i = 0; i < 175; ++i) this.simulation.tick();
		},
		data: () => {
			this.update.force();
	...
	private attach() {
		...
		this.d3updatePattern = new D3UpdatePattern({
			...
			create: (s) =>
				s
					...
					.classed(
						...
						(d) =>
							d.precalc.icon &&
							navigator.platform.includes("inux"),
					)
			...
			updateColor: (s) =>
				s.style("fill", (d) => d.pathes && d.pathes.labelcolor),
			updateTransform: (s) =>
		...
		this.d3updatePattern2 = new D3UpdatePattern({
			...
			updateTransform: (s) =>
				s
					...
					.attr(
						"x2",
						(d) => (d.forcepoints2 && d.forcepoints2.x) || 0,
					)
					.attr(
						"y2",
						(d) => (d.forcepoints2 && d.forcepoints2.y) || 0,
					),
			//.text(                   this.args.text)
		});
	}
