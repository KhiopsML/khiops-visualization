...
export class StemLayer implements ILayer {
	...
	private attach() {
		...
		this.d3updatePattern = new D3UpdatePattern({
			...
			clip: this.args.clip,
			data: () =>
				this.view.hypertree.data ? [this.view.hypertree.data] : [],
			name: "stem-link",
			...
			updateTransform: (s) => {
				if (straincurvature === "l")
					s.attr("x1", (d) => this.args.nodePos(d).re)
						...
						.attr("y2", (d) => 1)
						.attr("stroke-width", (d) => this.args.width(d) + 0.01)
						.attr("stroke-linecap", (d) => "round");
				else
					s.attr("d", (d) => this.arcOptions[this.args.curvature](d))
						.attr("stroke-width", (d) => this.args.width(d) + 0.015)
						.attr("stroke-linecap", (d) => "round");
			},
		});
