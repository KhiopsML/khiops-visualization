...
export class ImageLayer implements ILayer {
	...
	constructor(view: ILayerView, args: ImageLayerArgs) {
		this.view = view;
		this.args = args;
		this.args.width = this.args.width || 0.05;
		this.args.height = this.args.height || 0.05;
		this.name = args.name;
	}
	...
	private attach() {
		this.d3updatePattern = new D3UpdatePattern({
			...
			create: (s) =>
				s
					.attr("xlink:href", (d) => this.args.imagehref(d))
					.attr("width", this.args.width)
					.attr("height", this.args.height),
			updateColor: (s) => {},
			updateTransform: (s) =>
				s.attr("transform", (d, i, v) => {
					//const delta = this.args.delta(d, i, v)
					const delta = CmulR(
						{ re: -this.args.width, im: -this.args.height },
						d.distScale / 2,
					);

					return (
						` translate(${delta.re} ${delta.im})` +
