...
export class UnitDiskNav implements IUnitDisk {
	...
	constructor(view: UnitDiskView, args: UnitDiskArgs) {
		...
		var navTransformation = new NegTransformation(
		...
		this.navParameter = new UnitDisk(
			...
			},
			{
				...
				cacheUpdate: (ud: UnitDisk, cache: TransformationCache) => {
					...
					const spr = 1.08;
					setCacheZ(
						cache.unculledNodes[0],
						CmulR(args.transformation.state.P, -1),
					);
					setCacheZ(
						cache.unculledNodes[1],
						CmulR(args.transformation.state.θ, -spr),
					);
					setCacheZ(
						cache.unculledNodes[2],
						CptoCk({
