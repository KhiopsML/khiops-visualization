...
export class LayerStack {
  ...
  private updateLayers(): void {
    ...
    for (var layerfactoryfunc of this.args.unitdisk.args.layers) {
      ...
      if (this.args.unitdisk.args.layerOptions)
        mergeDeep(
          layer.args,
          this.args.unitdisk.args.layerOptions[layer.name] || {},
        );
      this.layers[layer.name] = layer;
    }

    this.mainSvgGroup.selectAll('*').remove();
    for (var l in this.layers) this.layers[l].update.parent();
  }
  ...
  private updatePath() {
    ...
    if (this.layers['stem-arc']) this.layers['stem-arc'].update.style();
